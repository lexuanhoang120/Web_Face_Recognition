...
import { getAll as getUsers } from "@/apis/user.api";

// TODO edit after fix the bug - issue #30249
export default function CAutoCompleteInfinte(
  {
    placeholder,
    error,
    errorText,
  ...
  useEffect( async () => {
    const items = await getUsers('', 1);
    setOptions(items?.data);
    // options.push(...items?.data)
    setPage(1);
  }, []);
  ...
  const loadMoreResults = async () => {
    ...
    const items = await getUsers('', nextPage);
    setOptions([...options, ...items?.data]);
    // options.push(...items?.data)
  };
  ...
  return (
    ...
    <MuiAutocomplete
      ...
      getOptionLabel={(option) => {
        return (
         (option?.name || option?.realname)  ? (option?.name ? option?.name : option?.realname) : option
        )
      }}
